...
const channels = [
  ...
  { icon:'Ãâ‚¬', cls:'purple', name:'Discrete Mathematics', sub:'Mathematical Foundations', access:'Public', members:180, desc:'Proofs, logic, sets, and more. Ask questions and grow together.' },
...
const posts = [
  { id:0, initials:'AS', author:'Arjun Sharma', ago:'2h', course:'CS301 Algorithms', icon:'</>', text:'In DijkstraÃ¢â‚¬â„¢s algorithm, why do we mark a vertex as Ã¢â‚¬Å“visitedÃ¢â‚¬Â when itÃ¢â‚¬â„¢s popped from the priority queue? Could we mark it when we push it instead?', comments:12 },
  { id:1, initials:'MI', avatar:'green', author:'Meera Iyer', ago:'4h', course:'Database Systems', icon:'DB', text:'HereÃ¢â‚¬â„¢s a quick cheatsheet for SQL joins and constraints. Hope this helps!', comments:8, file:{name:'Database-Cheatsheet.pdf',meta:'842 KB Ã¢â‚¬Â¢ 2 pages'} },
  { id:2, initials:'RV', avatar:'purple', author:'Rahul Verma', ago:'6h', course:'Discrete Mathematics', icon:'Ãâ‚¬', text:'Quick explanation of Proof by Contradiction with examples.', comments:15, audio:true },
  { id:3, initials:'AR', avatar:'green', author:'Ananya Rao', ago:'9h', course:'CS301 Algorithms', icon:'</>', text:'Sharing my handwritten notes on Dynamic Programming Ã¢â‚¬â€ knapsack problems.', comments:9, note:true },
];

const people = [
  {initials:'AM',name:'Alex Mensah',info:'Computer Science Ã¢â‚¬Â¢ 3rd Year',status:'Online'},
  {initials:'MI',name:'Meera Iyer',info:'DBMS Ã¢â‚¬Â¢ 2nd Year',status:'Dijkstra University'},
  {initials:'RV',name:'Rahul Verma',info:'Discrete Math Ã¢â‚¬Â¢ 2nd Year',status:'Dijkstra University'},
  {initials:'AR',name:'Ananya Rao',info:'Algorithms Ã¢â‚¬Â¢ 3rd Year',status:'Dijkstra University'},
];

const friendRequests = [
  { initials:'KG', name:'Karan Gupta', info:'Computer Science Ã‚Â· 2nd Year', status:'Dijkstra University' },
  { initials:'NS', name:'Neha Singh', info:'Mathematics Ã‚Â· 2nd Year', status:'Dijkstra University' },
];

const discoverPeople = [
  { initials:'VP', name:'Vikram Patel', info:'Computer Science Ã‚Â· 1st Year', status:'Dijkstra University' },
  { initials:'IN', name:'Ishita Nair', info:'DBMS Ã‚Â· 2nd Year', status:'Dijkstra University' },
  { initials:'AM', name:'Aditya Mehta', info:'Discrete Math Ã‚Â· 1st Year', status:'Dijkstra University' },
];
...
const discussionComments = {
  ...
  1: [{ initials:'AS', name:'Arjun Sharma', ago:'1h ago', text:'This is a great summary Ã¢â‚¬â€ thank you for sharing it.' }],
};
...
const chats = [
  ...
  { person:people[0], time:'9:22', preview:'Just joined and bookmarked. YouÃ¢â‚¬â„¢re the best!', messages:[
    ['mine','Hey Alex! Do you have the database normalization notes we discussed in class?','9:15'],
    ['theirs','Hey! Yeah, IÃ¢â‚¬â„¢ve uploaded the full notes. They cover 1NF to BCNF with examples.','9:16'],
    ['mine','Thanks! This will help a lot Ã°Å¸â„¢Å’','9:17'],
    ['theirs','Also, check out the short summary I recorded on functional dependencies.','9:18'],
    ['mine','Awesome! Just listened, super clear Ã°Å¸â€Â¥','9:20'],
  ]},
  { person:people[1], time:'8:44', preview:'I added the PDF to our channel.', messages:[['theirs','I added the PDF to our channel.','8:44']] },
  { person:people[2], time:'Tue', preview:'Ready for tomorrowÃ¢â‚¬â„¢s study session?', messages:[['mine','Ready for tomorrowÃ¢â‚¬â„¢s study session?','Tue']] },
...
function postCard(post) {
  ...
  return `<article class="card feed-card" data-post="${escapeHtml(post.id)}">
    <div class="post-head">${avatar({ ...post, photoURL:post.authorPhotoURL||post.photoURL },post.avatar||'')}<div class="post-who"><button class="post-author" data-profile="${people.findIndex(person=>person.id===post.authorId)}">${escapeHtml(post.author)}</button><div class="post-meta">${escapeHtml(relativeTime(post.createdAt||post.ago))}</div></div><button class="course-label" data-open-channel="${channelIndex}" aria-label="Open ${escapeHtml(post.course)}"><span class="dot-icon">${escapeHtml(post.icon)}</span><span>From <b>#${escapeHtml(post.course)}</b></span>${icon('chevron')}</button><button class="action" data-action="more" aria-label="More">${icon('more')}</button></div>
    ...
    ${post.audioURL?`<audio class="post-audio" controls src="${post.audioURL}"></audio>`:post.audio?`<div class="audio"><button class="play" data-action="play" aria-label="Play voice note">Ã¢â€“Â¶</button><div class="wave"></div><span class="muted small">01:45</span></div>`:''}
    ${post.note?`<div class="file-card" style="background:#fffaf0;min-height:100px;justify-content:center;color:#7d694c"><strong>0/1 Knapsack</strong><span class="muted"> recurrence Ã¢â€ â€™ dp[i][w] = max(Ã¢â‚¬Â¦)</span></div>`:''}
...
function messagesPageLegacy() {
  ...
  return shell(`<div class="messages-layout ${state.chatOpen?'chat-open':''}">
    ...
    <section class="chat-pane"><header class="chat-head"><button class="icon-btn back-mobile" data-action="back-chats">${icon('arrow')}</button>${avatar(chat.person)}<div><strong>${chat.person.name}</strong><div class="small" style="color:var(--green)">${chat.person.status}</div></div><button class="action push">${icon('search')}</button><button class="action">${icon('more')}</button></header><div class="chat-messages" id="chat-messages">${chat.messages.map(m=>`<div class="bubble ${m[0]}">${m[1]}<span class="bubble-time">${m[2]} ${m[0]==='mine'?'Ã¢Å“â€œÃ¢Å“â€œ':''}</span></div>`).join('')}</div><form class="chat-compose" id="chat-form"><button type="button" class="action">${icon('paperclip')}</button><input id="message-input" autocomplete="off" placeholder="Write a message..." /><button class="send-btn" aria-label="Send">${icon('send')}</button></form></section>
  </div>`,'Messages',false);
}

function friendsPageLegacy() {
  return shell(`<div class="stack"><div class="segment" style="grid-template-columns:repeat(3,1fr)"><button class="active">Friends</button><button data-action="requests">Requests Ã‚Â· 2</button><button data-action="discover">Discover</button></div><label class="search-box">${icon('search')}<input placeholder="Search people" /></label><div class="people-grid">${people.map((p,i)=>`<article class="card person-card">${avatar(p,['','green','purple','green'][i])}<div class="person-info"><strong>${p.name}</strong><span>${p.info}</span><span>${p.status}</span></div><button class="icon-btn" data-person-chat="${i}" aria-label="Message">${icon('chat')}</button><button class="action">${icon('more')}</button></article>`).join('')}</div><section class="card section-card"><div class="section-title"><h3>People you may know</h3><button class="link-btn">See all</button></div>${['Karan Gupta','Neha Singh','Vikram Patel'].map((n,i)=>`<div class="mini-channel">${avatar({initials:n.split(' ').map(x=>x[0]).join('')},i===1?'green':'')}<div class="channel-body"><strong>${n}</strong><span>Computer Science Ã¢â‚¬Â¢ Dijkstra University</span></div><button class="secondary" data-action="add-friend">+ Add friend</button></div>`).join('')}</section></div>`,'Friends');
}
...
function profilePage() {
  ...
  const studyLine=[state.profileCourse,state.profileYear].filter(Boolean).join(' Ã‚Â· ')||'Add your course and year level';
...
function channelsPage() {
  const list=channels.filter((c,i)=>(state.channelMode==='discover'||state.joined.has(i))&&c.name.toLowerCase().includes(state.channelSearch.toLowerCase()));
  return shell(`<div class="stack">
    <section class="card section-card saved-channel-card"><div class="section-title"><div style="display:flex;align-items:center;gap:10px"><div class="channel-icon green">${icon('bookmark')}</div><div><h3 style="margin:0">Saved Messages</h3><div class="muted small">Your private saved items</div></div></div><button class="primary" data-saved-chat>Open</button></div></section>
    <div style="display:flex;gap:12px"><div class="segment" style="flex:1"><button class="${state.channelMode==='joined'?'active':''}" data-channel-mode="joined">Joined</button><button class="${state.channelMode==='discover'?'active':''}" data-channel-mode="discover">Discover</button></div><button class="icon-btn" data-action="create-channel" aria-label="Create channel">${icon('plus')}</button></div>
    <label class="search-box">${icon('search')}<input id="channel-search" placeholder="Search channels" value="${state.channelSearch}" /></label>
    <div class="stack">${list.length?list.map((raw)=>{const c={...raw,name:escapeHtml(raw.name),sub:escapeHtml(raw.sub),access:escapeHtml(raw.access),desc:escapeHtml(raw.desc),icon:escapeHtml(raw.icon),cls:escapeHtml(raw.cls)};const idx=channels.indexOf(raw),joined=!state.isGuest&&state.joined.has(idx);return `<article class="card channel-card"><div class="channel-icon ${c.cls}">${c.icon}</div><div class="channel-copy"><h3 data-open-channel="${idx}" role="button" tabindex="0">${c.name}</h3><div class="muted">${c.sub}</div><div class="channel-tags"><span class="public">Ã¢â€”â€° ${c.access}</span><span>Ã¢â‚¬Â¢</span><span>${raw.members} members</span></div><p>${c.desc}</p></div>${joined?`<div class="channel-actions"><button class="secondary" data-open-channel="${idx}">Open</button><button class="action leave" data-leave="${idx}">Leave</button></div>`:`<button class="primary" data-join="${idx}">Join</button>`}</article>`}).join(''):`<div class="card empty"><div class="channel-icon">?</div><h3>No channels found</h3><p>Try a different name or browse all available channels.</p></div>`}</div>
  </div>`,'Channels',false);
}
...
function messageBubbleLegacy(message) {
  if (!Array.isArray(message) && message.type==='forwarded-post') {
    ...
    return `<div class="bubble ${message.side} forwarded-bubble"><div class="forwarded-label">${icon('share')} Forwarded post</div><div class="forwarded-post"><strong>From #${post.course}</strong><span class="muted small">${post.author}</span><p>${post.text}</p>${post.file?`<div class="forwarded-file"><span class="file-icon">PDF</span><span><b>${post.file.name}</b><small>${post.file.meta}</small></span></div>`:''}${post.audio?`<div class="forwarded-audio">${icon('paperclip')} Voice note</div>`:''}</div><span class="bubble-time">${message.time} Ã¢Å“â€œÃ¢Å“â€œ</span></div>`;
  }
  return `<div class="bubble ${escapeHtml(message[0])}">${escapeHtml(message[1])}<span class="bubble-time">${escapeHtml(message[2])} ${message[0]==='mine'?'Ã¢Å“â€œÃ¢Å“â€œ':''}</span></div>`;
}

function messageBubble(message) {
  if (!Array.isArray(message) && message.type==='post-link') {
    const deleteButton=state.activeChat===0&&message.side==='mine'&&message.id?`<button class="message-delete" data-delete-message="${escapeHtml(message.id)}">Delete</button>`:'';
    return `<div class="bubble ${message.side} forwarded-bubble"><div class="forwarded-label">${icon('share')} Forwarded post</div><button class="post-link-card" data-open-post="${escapeHtml(message.postId)}">${icon('chevron')} Open original post</button><div class="bubble-footer">${deleteButton}<span class="bubble-time">${escapeHtml(message.time)} ${message.side==='mine'?'&#10003;':''}</span></div></div>`;
  }
  if (!Array.isArray(message) && message.type==='attachment') {
    const file=message.file||{}; const fileUrl=safeAssetUrl(file.url);
    const deleteButton=state.activeChat===0&&message.side==='mine'&&message.id?`<button class="message-delete" data-delete-message="${escapeHtml(message.id)}">Delete</button>`:'';
    const shareButton=message.side==='mine'&&message.id?`<button class="message-share" data-forward-message="${escapeHtml(message.id)}">Forward</button>`:'';
    return `<div class="bubble ${message.side} forwarded-bubble"><div class="forwarded-label">${icon('paperclip')} Saved file</div>${fileUrl?`<a class="forwarded-file" href="${fileUrl}" download="${escapeHtml(file.name||'download')}"><span class="file-icon">${file.type==='application/pdf'?'PDF':'FILE'}</span><span><b>${escapeHtml(file.name||'Attachment')}</b><small>${escapeHtml(file.meta||'')}</small></span>${icon('download')}</a>`:''}<div class="bubble-footer">${shareButton}${deleteButton}<span class="bubble-time">${escapeHtml(message.time)} ${message.side==='mine'?'&#10003;':''}</span></div></div>`;
  }
  if (!Array.isArray(message) && message.type==='forwarded-post') {
    let post=message.post||{};
    post={...post,course:escapeHtml(post.course),author:escapeHtml(post.author),text:escapeHtml(post.text),file:post.file?{...post.file,name:escapeHtml(post.file.name),meta:escapeHtml(post.file.meta)}:null};
    ...
    const deleteButton=state.activeChat===0&&message.side==='mine'&&message.id?`<button class="message-delete" data-delete-message="${escapeHtml(message.id)}">Delete</button>`:'';
    return `<div class="bubble ${message.side} forwarded-bubble"><div class="forwarded-label">${icon('bookmark')} Saved post</div><div class="forwarded-post"><strong>From #${post.course}</strong><span class="muted small">${post.author}</span>${post.text?`<p>${post.text}</p>`:''}${imageUrl?`<img class="saved-post-image" src="${imageUrl}" alt="Saved post attachment" data-action="zoom-image" />`:''}${fileMarkup}${audioUrl?`<audio class="post-audio" controls src="${audioUrl}"></audio>`:''}</div><div class="bubble-footer">${deleteButton}<span class="bubble-time">${escapeHtml(message.time)} ${message.side==='mine'?'&#10003;':''}</span></div></div>`;
  }
  const deleteButton=state.activeChat===0&&message[0]==='mine'&&message[3]?`<button class="message-delete" data-delete-message="${escapeHtml(message[3])}">Delete</button>`:'';
  return `<div class="bubble ${escapeHtml(message[0])}">${escapeHtml(message[1])}<div class="bubble-footer">${deleteButton}<span class="bubble-time">${escapeHtml(message[2])} ${message[0]==='mine'?'&#10003;':''}</span></div></div>`;
}
...
function userProfilePage() {
  ...
  return shell(`<div class="stack"><button class="back-link" data-nav="friends">${icon('arrow')} Back to friends</button><section class="card profile-hero">${avatar(user,state.profileUser===1?'green':'')}<div><h2>${user.name}</h2><div class="muted">${user.info}</div><p>${user.status} Ã‚Â· StudyLoop member</p></div><button class="primary" data-person-chat="${state.profileUser}">Message</button></section><section class="card section-card"><div class="section-title"><h3>${user.name.split(' ')[0]}Ã¢â‚¬â„¢s channels</h3><span class="muted small">${userChannels.length} channels</span></div>${userChannels.map(idx=>{const c=channels[idx],joined=state.joined.has(idx);return `<div class="mini-channel"><div class="channel-icon ${c.cls}">${c.icon}</div><div class="channel-body"><strong>${c.name}</strong><span>${c.sub} Ã‚Â· ${c.members} members</span></div>${joined?`<button class="secondary" data-open-channel="${idx}">Open</button>`:`<button class="primary" data-join="${idx}">Join</button>`}</div>`}).join('')}</section><button class="secondary danger-action" data-action="logout">Log out</button></div>`,'Profile',false);
}
...
function channelDetailPage() {
  ...
  const body=state.channelTab==='files'
    ? (files.length?files.map(p=>`<div class="card file-row"><div class="file-icon">PDF</div><div class="file-info"><strong>${p.file.name}</strong><span>${p.file.meta} Ã‚Â· From ${p.author}</span></div><button class="download" data-action="download">${icon('download')}</button></div>`).join(''):`<div class="card empty"><div class="channel-icon">${icon('paperclip')}</div><h3>No files yet</h3><p>Files shared in this channel will appear here.</p></div>`)
  ...
  return shell(`<div class="stack"><button class="back-link" data-nav="channels">${icon('arrow')} Back to channels</button><section class="card channel-hero"><div class="channel-icon ${channel.cls}">${channel.icon}</div><div class="channel-hero-copy"><h2>${channel.name}</h2><p>${channel.desc}</p><div class="channel-tags"><span class="public">Ã¢â€”â€° ${channel.access}</span><span>Ã¢â‚¬Â¢</span><span>${channel.members} members</span><span>Ã¢â‚¬Â¢</span><span>${channel.sub}</span></div></div>${joined?`<button class="secondary" data-leave="${state.activeChannel}">Leave</button>`:`<button class="primary" data-join="${state.activeChannel}">Join</button>`}</section><div class="channel-tabs"><button class="${state.channelTab==='posts'?'active':''}" data-channel-tab="posts">Posts</button><button class="${state.channelTab==='files'?'active':''}" data-channel-tab="files">Files</button><button class="${state.channelTab==='members'?'active':''}" data-channel-tab="members">Members</button></div><div class="stack">${body}</div>${joined&&state.channelTab==='posts'?`<button class="new-post-bar" data-action="channel-post"><span class="channel-icon">${icon('plus')}</span> Start a new post in ${channel.name}Ã¢â‚¬Â¦</button>`:''}</div>`,'Channel',false);
}

function friendsPageLegacy2() {
  return shell(`<div class="stack"><div class="segment" style="grid-template-columns:repeat(3,1fr)"><button class="active">Friends</button><button data-action="requests">Requests Ã‚Â· 2</button><button data-action="discover">Discover</button></div><label class="search-box">${icon('search')}<input placeholder="Search people" /></label><div class="people-grid">${people.map((p,i)=>`<article class="card person-card" data-profile="${i}">${avatar(p,['','green','purple','green'][i])}<div class="person-info"><strong>${p.name}</strong><span>${p.info}</span><span>${p.status}</span></div><button class="icon-btn" data-person-chat="${i}" aria-label="Message">${icon('chat')}</button><button class="action">${icon('more')}</button></article>`).join('')}</div><section class="card section-card"><div class="section-title"><h3>People you may know</h3><button class="link-btn">See all</button></div>${['Karan Gupta','Neha Singh','Vikram Patel'].map((n,i)=>`<div class="mini-channel">${avatar({initials:n.split(' ').map(x=>x[0]).join('')},i===1?'green':'')}<div class="channel-body"><strong>${n}</strong><span>Computer Science Ã‚Â· Dijkstra University</span></div><button class="secondary" data-action="add-friend">+ Add friend</button></div>`).join('')}</section></div>`,'Friends');
}

function friendsPage() {
  const tabs=[['friends','Friends'],['requests',`Requests Ã‚Â· ${friendRequests.filter((_,i)=>!state.acceptedRequests.has(i)).length}`],['discover','Discover']];
  ...
  const discoverCards=discoverPeople.filter(p=>`${p.name} ${p.info} ${p.status}`.toLowerCase().includes(state.peopleSearch.toLowerCase())).map((p,i)=>`<article class="card person-card"><div class="avatar ${i===1?'green':'purple'}">${p.initials}</div><div class="person-info"><strong>${p.name}</strong><span>${p.info}</span><span>${p.status}</span></div><button class="${state.sentRequests.has(i)?'secondary':'primary'}" data-discover-add="${i}" ${state.sentRequests.has(i)?'disabled':''}>${state.sentRequests.has(i)?'Request sent':'Add friend'}</button></article>`).join('');
...
function postDetailPage() {
  ...
  return shell(`<div class="stack"><button class="back-link" data-nav="home">${icon('arrow')} Back to feed</button>${postCard(post)}<section class="card discussion-card"><div class="section-title"><h3>Discussion Ã‚Â· ${comments.length} comments</h3><button class="link-btn">Newest</button></div><div class="comment-list">${comments.map(c=>`<article class="comment"><div class="avatar">${c.initials}</div><div class="comment-copy"><div><strong>${c.name}</strong><span>${c.ago}</span></div><p>${c.text}</p><button data-action="reply" class="link-btn">Reply</button></div></article>`).join('')}</div><form id="comment-form" class="comment-compose"><input name="comment" required autocomplete="off" placeholder="Write a commentÃ¢â‚¬Â¦" /><button class="send-btn" aria-label="Post comment">${icon('send')}</button></form></section></div>`,'Discussion',false);
}

function notificationsPage() {
  const content=notifications.length?notifications.map(n=>`<article class="card notification-item ${state.readNotifications.has(n.id)?'read':''}"><div class="notification-icon">${icon(n.icon)}</div><div><strong>${n.title}</strong><p>${n.body}</p><span>${n.time}</span></div>${!state.readNotifications.has(n.id)?'<i></i>':''}</article>`).join(''):`<div class="card empty"><div class="notification-icon">${icon('bell')}</div><h3>No notifications</h3><p class="muted">YouÃ¢â‚¬â„¢re all caught up. New activity will appear here.</p></div>`;
...
function postComposerModal() {
  ...
  const premium=true;
  return `<div class="modal-backdrop" data-action="close-modal"><form class="modal" id="post-form"><div class="modal-head"><div><h2>New post</h2><p class="muted" style="margin:4px 0 0">Posting in #${displayChannelName}</p></div><button type="button" class="icon-btn" data-action="close-modal">${icon('x')}</button></div><div class="field"><label>Text</label><textarea name="postText" maxlength="1000" placeholder="Ask a question, share a note, or start a discussionÃ¢â‚¬Â¦"></textarea></div>${premium?`<div class="field"><label>Attachments</label><input type="file" name="assets" multiple accept="image/*,audio/*,application/pdf" /><span class="muted small">You can combine text, an image, a file, and a voice note.</span></div><div class="voice-recorder"><button type="button" class="secondary" data-action="record-voice">${icon('chat')} Record voice note</button><span class="muted small" id="voice-status">Not recording</span><audio id="voice-preview" controls hidden></audio></div>`:`<div class="card" style="padding:12px;background:var(--tg-blue-pale);color:var(--tg-blue-dark)">All accounts can publish text, images, files, and voice notes. Each upload is limited to 50 MB.</div>`}<div class="modal-actions"><button type="button" class="secondary" data-action="close-modal">Cancel</button><button class="primary">Publish post</button></div></form></div>`;
}
...
document.addEventListener('click',e=>{
  ...
  if(deleteMessage){
    ...
    if(state.activeChat===0&&messageId){
  ...
  if(action.dataset.action==='delete-post'&&!post){const menu=action.closest('[data-post-id]');const item=posts.find(p=>String(p.id)===String(menu?.dataset.postId));if(item?.authorId===state.userId){deleteCloudPost(item.id).then(()=>{const index=posts.indexOf(item);if(index>=0)posts.splice(index,1);menu?.remove();render();notify('Post deleted');}).catch(error=>notify(userFacingError(error,'Unable to delete post.')));}return;}
  ...
  if(action.dataset.action==='upload-saved'||action.dataset.action==='upload-file'){const input=$('#file-upload');input.accept='image/jpeg,image/png,image/webp,audio/*,application/pdf';input.click();return;}
  ...
  if(action.dataset.action==='record-voice'){
    ...
    navigator.mediaDevices.getUserMedia({ audio:true }).then(stream=>{
      ...
      activeRecorder.onstop=()=>{state.recordedAudio=new Blob(chunks,{type:activeRecorder.mimeType||'audio/webm'});stream.getTracks().forEach(track=>track.stop());if(preview){preview.src=URL.createObjectURL(state.recordedAudio);preview.hidden=false;}clearInterval(recordingTimer);const seconds=Math.max(1,Math.round((Date.now()-recordingStartedAt)/1000));state.recordedAudioDuration=seconds;if(status)status.textContent='Voice note ready Ã‚Â· '+Math.floor(seconds/60)+':'+String(seconds%60).padStart(2,'0');action.textContent='Record again';};
      activeRecorder.start(); recordingStartedAt=Date.now(); if(status)status.textContent='Recording 0:00 Ã‚Â· tap to stop'; recordingTimer=setInterval(()=>{const seconds=Math.floor((Date.now()-recordingStartedAt)/1000);if(status)status.textContent='Recording '+Math.floor(seconds/60)+':'+String(seconds%60).padStart(2,'0')+' Ã‚Â· tap to stop';},250); action.textContent='Stop recording';
  ...
  if(action.dataset.action==='save'&&post){const id=String(post.dataset.post);const item=posts.find(entry=>String(entry.id)===id);const willSave=!state.saved.has(id);const update=setSavedPost(state.userId,id,willSave);const savedPost=item?{id:String(item.id),course:item.course,author:item.author,authorId:item.authorId||'',text:item.text||'',imageURL:item.imageURL||null,audioURL:item.audioURL||null,file:item.file||null}:null;const addToChat=willSave&&savedPost?saveCloudMessage({conversationId:`saved_${state.userId}`,participants:[state.userId],senderId:state.userId,type:'savedPost',text:'Saved post',postId:id,post:savedPost}):Promise.resolve();Promise.all([update,addToChat]).then(()=>notify(willSave?'Saved to Saved Messages':'Bookmark removed')).catch(error=>notify(userFacingError(error,'Unable to update Saved Messages.')));}
  else if(action.dataset.action==='share'&&post){state.sharePostId=String(post.dataset.post);document.body.insertAdjacentHTML('beforeend',shareModal(state.sharePostId));}
  else if(action.dataset.action==='download') notify('Download started');
  else if(action.dataset.action==='play'){action.textContent=action.textContent==='Ã¢ÂÅ¡Ã¢ÂÅ¡'?'Ã¢â€“Â¶':'Ã¢ÂÅ¡Ã¢ÂÅ¡';notify(action.textContent==='Ã¢ÂÅ¡Ã¢ÂÅ¡'?'Playing voice note':'Voice note paused');}
    ...
    else if (isBackdrop && e.target === action) action.remove();
  }
...
document.addEventListener('submit',async e=>{
  ...
  if(e.target.id==='chat-form'){e.preventDefault();if(guestNeedsSignIn('Sign in to send messages.'))return;const input=$('#message-input');const value=input.value.trim();if(!value)return;const participants=state.activeChat===0?[state.userId]:[state.userId,people[state.activeChat-1]?.id].filter(Boolean);const chat=chats[state.activeChat]||chats[0];chat.messages.push(['mine',value,new Date().toLocaleTimeString([],{hour:'2-digit',minute:'2-digit'})+' Ã¢Å“â€œ']);chat.preview=value;input.value='';render();saveCloudMessage({conversationId:conversationIdFor(),participants,senderId:state.userId,type:'text',text:value}).catch(error=>notify(userFacingError(error,'Unable to send message.')));}
...
document.addEventListener('change',async e=>{
  ...
  const allowed=file.type==='application/pdf'||file.type.startsWith('image/')||file.type.startsWith('audio/');
  ...
  try {
    ...
    const url=await uploadAsset(file,`users/${state.userId}/saved/${storedName}`);
    await saveCloudMessage({conversationId:`saved_${state.userId}`,participants:[state.userId],senderId:state.userId,type:'attachment',text:'Saved a file',file:{name:file.name,meta:size,type:file.type,url}});
    notify('File saved to Saved Messages');
...
document.addEventListener('change',async e=>{
  ...
  if(e.target.id==='file-upload'){
    ...
    const size=file.size<1024*1024?`${Math.max(1,Math.round(file.size/1024))} KB`:`${(file.size/(1024*1024)).toFixed(1)} MB`;
    const chat=chats[state.activeChat]||chats[0];
    try { const url=await uploadAsset(file,`users/${state.userId}/saved/${safeStorageName(file)}`); chat.messages.push(['mine',`File uploaded: ${file.name} Ã‚Â· ${size} (${url})`,'Now']); chat.preview=`Uploaded ${file.name}`; notify('File saved'); } catch(error) { notify(error.message||'Unable to upload your file.'); } finally { e.target.value=''; render(); }
  }
});
...
async function subscribeActiveMessages() {
  ...
  try { stopActiveMessages=await observeMessages(conversationId, messages=>{const chat=chats[state.activeChat];if(!chat)return;const timeOf=message=>message.createdAt?.toDate?message.createdAt.toDate().toLocaleTimeString([],{hour:'2-digit',minute:'2-digit'}):'Now';chat.messages=messages.map(message=>{const side=message.senderId===state.userId?'mine':'theirs';if(message.type==='postLink')return{id:message.id,type:'post-link',side,time:timeOf(message),postId:message.postId};if(message.type==='savedPost')return{id:message.id,type:'forwarded-post',side,time:timeOf(message),post:message.post};if(message.type==='attachment')return{id:message.id,type:'attachment',side,time:timeOf(message),file:message.file};return[side,message.text||'',timeOf(message),message.id];});chat.preview=messages.at(-1)?.text||'Saved item';render();},error=>console.warn('Unable to load messages',error)); } catch(error) { console.warn('Unable to subscribe to messages',error); }
}
