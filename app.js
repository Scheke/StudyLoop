...
const attachmentSelectionError = files => {
  ...
  if(kinds.filter(kind=>kind==='image').length>1)return 'Use up to one image per post. You can include multiple documents.';
...
function postCard(post) {
  ...
  const imageUrl=safeAssetUrl(post.imageURL); const audioUrl=safeAssetUrl(post.audioURL);
...
async function deletePostAndAttachments(item) {
  ...
  await Promise.all([item.imageURL,item.audioURL,...postFiles(item).map(file=>file?.url)].filter(Boolean).map(url=>deleteUploadedAsset(url).catch(error=>console.warn('Unable to remove an attachment after deleting its post.',error))));
}
...
document.addEventListener('submit',async e=>{
  ...
  if(e.target.id==='channel-compose-form'){
    ...
    const uploaded=[];
    try{
      const id=crypto.randomUUID();const image=assets.find(file=>uploadKind(file)==='image');const audio=state.recordedAudio||assets.find(file=>uploadKind(file)==='audio');const documentFiles=assets.filter(file=>uploadKind(file)==='document');
      ...
      const post={id,initials:initials(state.profileName),author:state.profileName,authorId:state.userId,authorPhotoURL:state.profilePhotoURL,ago:'now',course:channel.name,channelId:channel.id,icon:channel.icon,text,comments:0,imageURL,audioURL,audioDuration:state.recordedAudioDuration||0,files};
