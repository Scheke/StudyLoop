...
document.addEventListener('click',e=>{
  ...
  if(action.dataset.action==='channel-options'){const channel=channels[state.activeChannel];if(!channel)return;const muted=state.mutedChannels.has(String(channel.id));document.body.insertAdjacentHTML('beforeend',`<div class="modal-backdrop" data-action="close-modal"><div class="post-menu"><h3>Channel settings</h3><button class="secondary" data-action="share-channel" data-channel-id="${escapeHtml(channel.id)}">${icon('share')} Share channel</button>${channel.ownerId===state.userId?`<button class="secondary" data-action="edit-channel">Edit channel</button>`:''}<button class="secondary" data-action="toggle-channel-mute" data-channel-id="${escapeHtml(channel.id)}">${muted?'Unmute notifications':'Mute notifications'}</button><button class="secondary" data-action="close-modal">Cancel</button></div></div>`);return;}
...
document.addEventListener('submit',async e=>{
  ...
  if(e.target.id==='edit-channel-form'){
    ...
    const data=new FormData(e.target);const name=String(data.get('name')||'').trim();const image=data.get('channelImage');const submit=e.target.querySelector('button.primary');submit.disabled=true;
    try{let imageURL=channel.imageURL||'';if(image instanceof File&&image.size){imageURL=await uploadAsset(image,`users/${state.userId}/channels/${channel.id}/avatar-${safeStorageName(image)}`);}await updateCloudChannel(channel.id,{name,imageURL});Object.assign(channel,{name,imageURL,icon:name.slice(0,2).toUpperCase()});e.target.closest('.modal-backdrop')?.remove();render();notify('Channel updated');}catch(error){submit.disabled=false;notify(userFacingError(error,'Unable to update this channel.'));}
  }
  ...
  if(e.target.id==='channel-compose-form'){
    ...
    if(assets.length>3||assets.some((file,index)=>file.size>200*1024*1024||!kinds[index])||new Set(kinds).size!==kinds.length){notify('Use up to one image, one document, and one audio file, each 200 MB or smaller. Videos are not accepted.');return;}
...
document.addEventListener('change',async e=>{
  ...
  if(e.target.name==='channelAssets'){
    ...
    if(files.length>3||files.some((file,index)=>file.size>200*1024*1024||!kinds[index])||new Set(kinds).size!==kinds.length){notify('Use up to one image, one document, and one audio file, each 200 MB or smaller. Videos are not accepted.');e.target.value='';return;}
...
function editChannelModal(channel){return `<div class="modal-backdrop" data-action="close-modal"><form class="modal" id="edit-channel-form"><div class="modal-head"><h2>Edit channel</h2><button type="button" class="icon-btn" data-action="close-modal">${icon('x')}</button></div><div class="field"><label>Channel name</label><input name="name" maxlength="80" required value="${escapeHtml(channel.name)}" /></div><div class="field"><label>Channel picture</label><input type="file" name="channelImage" accept="image/jpeg,image/png,image/webp" /><span class="muted small">JPEG, PNG, or WebP up to 10 MB.</span></div><div class="modal-actions"><button type="button" class="secondary" data-action="close-modal">Cancel</button><button class="primary">Save changes</button></div></form></div>`;}
