...
function shell(content,title,actions=true) {
  return `<div class="app-shell">
    ...
    <main class="main">
      <header class="topbar"><h1>StudyLoop</h1><div class="top-actions"><button class="pricing-header" data-nav="pricing">Plans</button>${state.isGuest?`<button class="guest-chip" data-action="sign-in">Guest · Sign in</button>`:''}${actions?`<button class="icon-btn" data-action="search" aria-label="Search">${icon('search')}</button>${notifications.some(item=>!state.readNotifications.has(item.id))?`<button class="icon-btn" data-action="notifications" aria-label="Notifications">${icon('bell')}<span class="notify-dot">${notifications.filter(item=>!state.readNotifications.has(item.id)).length}</span></button>`:`<button class="icon-btn" data-action="notifications" aria-label="Notifications">${icon('bell')}</button>`}`:''}</div></header>
...
function homePage() {
  const filtered=posts.filter(p=>state.feedFilter==='All'||p.course===state.feedFilter);
  const chips=['All',...channels.map(channel=>channel.name)];
  return shell(`<button class="mobile-saved-shortcut" data-nav="saved">${icon('bookmark')} Saved</button><div class="page-grid"><div class="stack">
...
function postDetailPage() {
  ...
  const commentHtml=comments.map(c=>`<article class="comment ${c.parentId?'comment-reply':''}">${avatar({name:c.name,photoURL:c.authorPhotoURL,initials:c.initials})}<div class="comment-copy"><div><strong>${escapeHtml(c.name)}</strong><span>${escapeHtml(c.ago||'now')}</span></div>${c.text?`<p>${escapeHtml(c.text)}</p>`:''}${safeAssetUrl(c.imageURL)?`<img class="comment-image" src="${safeAssetUrl(c.imageURL)}" alt="Comment attachment" data-action="zoom-image" />`:''}${safeAssetUrl(c.audioURL)?`<audio controls src="${safeAssetUrl(c.audioURL)}"></audio>`:''}${!c.parentId&&c.id?`<button class="link-btn" data-action="reply-comment" data-reply-comment="${escapeHtml(c.id)}">Reply</button>`:''}</div></article>`).join('');
