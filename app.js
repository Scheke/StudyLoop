import { signUp, signIn, report, saveCloudPost, deleteCloudPost, uploadAsset, observeAuth, observePosts, getUserProfile, updateUserProfile, observeChannels, createCloudChannel, observeUsers, observeMemberships, setMembership, observeSaved, setSavedPost, saveCloudMessage, observeMessages } from './firebase.js';
...
const channels = [
  ...
  { icon:'Ï€', cls:'purple', name:'Discrete Mathematics', sub:'Mathematical Foundations', access:'Public', members:180, desc:'Proofs, logic, sets, and more. Ask questions and grow together.' },
...
const posts = [
  { id:0, initials:'AS', author:'Arjun Sharma', ago:'2h', course:'CS301 Algorithms', icon:'</>', text:'In Dijkstraâ€™s algorithm, why do we mark a vertex as â€œvisitedâ€ when itâ€™s popped from the priority queue? Could we mark it when we push it instead?', comments:12 },
  { id:1, initials:'MI', avatar:'green', author:'Meera Iyer', ago:'4h', course:'Database Systems', icon:'DB', text:'Hereâ€™s a quick cheatsheet for SQL joins and constraints. Hope this helps!', comments:8, file:{name:'Database-Cheatsheet.pdf',meta:'842 KB â€¢ 2 pages'} },
  { id:2, initials:'RV', avatar:'purple', author:'Rahul Verma', ago:'6h', course:'Discrete Mathematics', icon:'Ï€', text:'Quick explanation of Proof by Contradiction with examples.', comments:15, audio:true },
  { id:3, initials:'AR', avatar:'green', author:'Ananya Rao', ago:'9h', course:'CS301 Algorithms', icon:'</>', text:'Sharing my handwritten notes on Dynamic Programming â€” knapsack problems.', comments:9, note:true },
];

const people = [
  {initials:'AM',name:'Alex Mensah',info:'Computer Science â€¢ 3rd Year',status:'Online'},
  {initials:'MI',name:'Meera Iyer',info:'DBMS â€¢ 2nd Year',status:'Dijkstra University'},
  {initials:'RV',name:'Rahul Verma',info:'Discrete Math â€¢ 2nd Year',status:'Dijkstra University'},
  {initials:'AR',name:'Ananya Rao',info:'Algorithms â€¢ 3rd Year',status:'Dijkstra University'},
];

const friendRequests = [
  { initials:'KG', name:'Karan Gupta', info:'Computer Science Â· 2nd Year', status:'Dijkstra University' },
  { initials:'NS', name:'Neha Singh', info:'Mathematics Â· 2nd Year', status:'Dijkstra University' },
];

const discoverPeople = [
  { initials:'VP', name:'Vikram Patel', info:'Computer Science Â· 1st Year', status:'Dijkstra University' },
  { initials:'IN', name:'Ishita Nair', info:'DBMS Â· 2nd Year', status:'Dijkstra University' },
  { initials:'AM', name:'Aditya Mehta', info:'Discrete Math Â· 1st Year', status:'Dijkstra University' },
];
...
const discussionComments = {
  ...
  1: [{ initials:'AS', name:'Arjun Sharma', ago:'1h ago', text:'This is a great summary â€” thank you for sharing it.' }],
};
...
const chats = [
  ...
  { person:people[0], time:'9:22', preview:'Just joined and bookmarked. Youâ€™re the best!', messages:[
    ['mine','Hey Alex! Do you have the database normalization notes we discussed in class?','9:15'],
    ['theirs','Hey! Yeah, Iâ€™ve uploaded the full notes. They cover 1NF to BCNF with examples.','9:16'],
    ['mine','Thanks! This will help a lot ðŸ™Œ','9:17'],
    ['theirs','Also, check out the short summary I recorded on functional dependencies.','9:18'],
    ['mine','Awesome! Just listened, super clear ðŸ”¥','9:20'],
  ]},
  { person:people[1], time:'8:44', preview:'I added the PDF to our channel.', messages:[['theirs','I added the PDF to our channel.','8:44']] },
  { person:people[2], time:'Tue', preview:'Ready for tomorrowâ€™s study session?', messages:[['mine','Ready for tomorrowâ€™s study session?','Tue']] },
...
function postCard(post) {
  ...
  return `<article class="card feed-card" data-post="${escapeHtml(post.id)}">
    ...
    ${post.audioURL?`<audio class="post-audio" controls src="${post.audioURL}"></audio>`:post.audio?`<div class="audio"><button class="play" data-action="play" aria-label="Play voice note">â–¶</button><div class="wave"></div><span class="muted small">01:45</span></div>`:''}
    ${post.note?`<div class="file-card" style="background:#fffaf0;min-height:100px;justify-content:center;color:#7d694c"><strong>0/1 Knapsack</strong><span class="muted"> recurrence â†’ dp[i][w] = max(â€¦)</span></div>`:''}
...
function messagesPageLegacy() {
  ...
  return shell(`<div class="messages-layout ${state.chatOpen?'chat-open':''}">
    ...
    <section class="chat-pane"><header class="chat-head"><button class="icon-btn back-mobile" data-action="back-chats">${icon('arrow')}</button>${avatar(chat.person)}<div><strong>${chat.person.name}</strong><div class="small" style="color:var(--green)">${chat.person.status}</div></div><button class="action push">${icon('search')}</button><button class="action">${icon('more')}</button></header><div class="chat-messages" id="chat-messages">${chat.messages.map(m=>`<div class="bubble ${m[0]}">${m[1]}<span class="bubble-time">${m[2]} ${m[0]==='mine'?'âœ“âœ“':''}</span></div>`).join('')}</div><form class="chat-compose" id="chat-form"><button type="button" class="action">${icon('paperclip')}</button><input id="message-input" autocomplete="off" placeholder="Write a messageâ€¦" /><button class="send-btn" aria-label="Send">${icon('send')}</button></form></section>
  </div>`,'Messages',false);
}

function friendsPageLegacy() {
  return shell(`<div class="stack"><div class="segment" style="grid-template-columns:repeat(3,1fr)"><button class="active">Friends</button><button data-action="requests">Requests Â· 2</button><button data-action="discover">Discover</button></div><label class="search-box">${icon('search')}<input placeholder="Search people" /></label><div class="people-grid">${people.map((p,i)=>`<article class="card person-card">${avatar(p,['','green','purple','green'][i])}<div class="person-info"><strong>${p.name}</strong><span>${p.info}</span><span>${p.status}</span></div><button class="icon-btn" data-person-chat="${i}" aria-label="Message">${icon('chat')}</button><button class="action">${icon('more')}</button></article>`).join('')}</div><section class="card section-card"><div class="section-title"><h3>People you may know</h3><button class="link-btn">See all</button></div>${['Karan Gupta','Neha Singh','Vikram Patel'].map((n,i)=>`<div class="mini-channel">${avatar({initials:n.split(' ').map(x=>x[0]).join('')},i===1?'green':'')}<div class="channel-body"><strong>${n}</strong><span>Computer Science â€¢ Dijkstra University</span></div><button class="secondary" data-action="add-friend">+ Add friend</button></div>`).join('')}</section></div>`,'Friends');
}
...
function profilePageLegacy2() {
  return shell(`<div class="stack"><section class="card profile-hero">${avatar({initials:'NS'})}<div><h2>Naledi Smith</h2><div class="muted">@naledi â€¢ Computer Science, 3rd Year</div><p>Learning in public, one study session at a time.</p></div><button class="secondary" style="margin-left:auto" data-action="edit-profile">Edit profile</button></section><div class="stat-row"><div class="stat"><strong>12</strong><span>Friends</span></div><div class="stat"><strong>4</strong><span>Channels</span></div><div class="stat"><strong>${state.saved.size}</strong><span>Saved</span></div></div><section class="card section-card"><div class="section-title"><h3>Account</h3></div>${[['user','Personal information'],['bell','Notifications'],['bookmark','Saved posts'],['users','Privacy & security']].map(r=>`<div class="settings-row">${icon(r[0])}<span>${r[1]}</span>${icon('chevron','i-right')}</div>`).join('')}</section></div>`,'Profile',false);
}
...
function profilePage() {
  ...
  const studyLine=[state.profileCourse,state.profileYear].filter(Boolean).join(' Â· ')||'Add your course and year level';
  return shell(`<div class="stack"><section class="card profile-hero">${currentUserAvatar()}<div><h2>${escapeHtml(state.profileName)}</h2><div class="muted">${escapeHtml(studyLine)}</div><p>${escapeHtml(state.profileBio||'Tell classmates a little about yourself.')}</p></div><button class="secondary" style="margin-left:auto" data-action="edit-profile">Edit profile</button></section><div class="stat-row"><div class="stat"><strong>${state.acceptedRequests.size}</strong><span>Friends</span></div><div class="stat"><strong>${state.joined.size}</strong><span>Channels</span></div><div class="stat"><strong>${state.saved.size}</strong><span>Saved</span></div></div><section class="card section-card"><div class="section-title"><h3>Account</h3></div>${accountRows.map(r=>`<button class="settings-row" data-setting="${escapeHtml(r[1])}">${icon(r[0])}<span>${escapeHtml(r[1])}</span>${icon('chevron','i-right')}</button>`).join('')}</section></div>`,'Profile',false);
}
...
function homePage() {
  ...
  return shell(`<div class="page-grid"><div class="stack">
    ...
    <section class="card section-card"><div class="section-title"><h3>Your week</h3><button class="link-btn">View activity</button></div><strong>68% learning goal</strong><div class="progress"><i></i></div><span class="muted small">4 of 6 study goals completed</span></section>
...
function messageBubble(message) {
  ...
  return `<div class="bubble ${escapeHtml(message[0])}">${escapeHtml(message[1])}<span class="bubble-time">${escapeHtml(message[2])} ${message[0]==='mine'?'âœ“âœ“':''}</span></div>`;
}
...
function userProfilePage() {
  ...
  return shell(`<div class="stack"><button class="back-link" data-nav="friends">${icon('arrow')} Back to friends</button><section class="card profile-hero">${avatar(user,state.profileUser===1?'green':'')}<div><h2>${user.name}</h2><div class="muted">${user.info}</div><p>${user.status} Â· StudyLoop member</p></div><button class="primary" data-person-chat="${state.profileUser}">Message</button></section><section class="card section-card"><div class="section-title"><h3>${user.name.split(' ')[0]}â€™s channels</h3><span class="muted small">${userChannels.length} channels</span></div>${userChannels.map(idx=>{const c=channels[idx],joined=state.joined.has(idx);return `<div class="mini-channel"><div class="channel-icon ${c.cls}">${c.icon}</div><div class="channel-body"><strong>${c.name}</strong><span>${c.sub} Â· ${c.members} members</span></div>${joined?`<button class="secondary" data-open-channel="${idx}">Open</button>`:`<button class="primary" data-join="${idx}">Join</button>`}</div>`}).join('')}</section></div>`,'Profile',false);
}
...
function channelDetailPage() {
  ...
  const body=state.channelTab==='files'
    ? (files.length?files.map(p=>`<div class="card file-row"><div class="file-icon">PDF</div><div class="file-info"><strong>${p.file.name}</strong><span>${p.file.meta} Â· From ${p.author}</span></div><button class="download" data-action="download">${icon('download')}</button></div>`).join(''):`<div class="card empty"><div class="channel-icon">${icon('paperclip')}</div><h3>No files yet</h3><p>Files shared in this channel will appear here.</p></div>`)
  ...
  return shell(`<div class="stack"><button class="back-link" data-nav="channels">${icon('arrow')} Back to channels</button><section class="card channel-hero"><div class="channel-icon ${channel.cls}">${channel.icon}</div><div class="channel-hero-copy"><h2>${channel.name}</h2><p>${channel.desc}</p><div class="channel-tags"><span class="public">â—‰ ${channel.access}</span><span>â€¢</span><span>${channel.members} members</span><span>â€¢</span><span>${channel.sub}</span></div></div>${joined?`<button class="secondary" data-leave="${state.activeChannel}">Leave</button>`:`<button class="primary" data-join="${state.activeChannel}">Join</button>`}</section><div class="channel-tabs"><button class="${state.channelTab==='posts'?'active':''}" data-channel-tab="posts">Posts</button><button class="${state.channelTab==='files'?'active':''}" data-channel-tab="files">Files</button><button class="${state.channelTab==='members'?'active':''}" data-channel-tab="members">Members</button></div><div class="stack">${body}</div>${joined&&state.channelTab==='posts'?`<button class="new-post-bar" data-action="channel-post"><span class="channel-icon">${icon('plus')}</span> Start a new post in ${channel.name}â€¦</button>`:''}</div>`,'Channel',false);
}
...
function friendsPage() {
  const tabs=[['friends','Friends'],['requests',`Requests Â· ${friendRequests.filter((_,i)=>!state.acceptedRequests.has(i)).length}`],['discover','Discover']];
...
function postDetailPage() {
  ...
  return shell(`<div class="stack"><button class="back-link" data-nav="home">${icon('arrow')} Back to feed</button>${postCard(post)}<section class="card discussion-card"><div class="section-title"><h3>Discussion Â· ${comments.length} comments</h3><button class="link-btn">Newest</button></div><div class="comment-list">${comments.map(c=>`<article class="comment"><div class="avatar">${c.initials}</div><div class="comment-copy"><div><strong>${c.name}</strong><span>${c.ago}</span></div><p>${c.text}</p><button data-action="reply" class="link-btn">Reply</button></div></article>`).join('')}</div><form id="comment-form" class="comment-compose"><input name="comment" required autocomplete="off" placeholder="Write a commentâ€¦" /><button class="send-btn" aria-label="Post comment">${icon('send')}</button></form></section></div>`,'Discussion',false);
}

function notificationsPage() {
  const content=notifications.length?notifications.map(n=>`<article class="card notification-item ${state.readNotifications.has(n.id)?'read':''}"><div class="notification-icon">${icon(n.icon)}</div><div><strong>${n.title}</strong><p>${n.body}</p><span>${n.time}</span></div>${!state.readNotifications.has(n.id)?'<i></i>':''}</article>`).join(''):`<div class="card empty"><div class="notification-icon">${icon('bell')}</div><h3>No notifications</h3><p class="muted">Youâ€™re all caught up. New activity will appear here.</p></div>`;
...
function render() {
  const pages={landing:landingPage,home:homePage,channels:channelsPage,messages:messagesPage,friends:friendsPage,saved:savedPage,pricing:pricingPage,profile:profilePage,settings:settingsPage,'user-profile':userProfilePage,'channel-detail':channelDetailPage,'post-detail':postDetailPage,notifications:notificationsPage};
...
document.addEventListener('click',e=>{
  ...
  if(action.dataset.action==='record-voice'){
    ...
    if(activeRecorder?.state==='recording'){activeRecorder.stop();action.textContent='Record voice note';return;}
    navigator.mediaDevices.getUserMedia({ audio:true }).then(stream=>{
      ...
      activeRecorder.onstop=()=>{state.recordedAudio=new Blob(chunks,{type:activeRecorder.mimeType||'audio/webm'});stream.getTracks().forEach(track=>track.stop());if(preview){preview.src=URL.createObjectURL(state.recordedAudio);preview.hidden=false;}if(status)status.textContent='Voice note ready';action.textContent='Record again';};
      activeRecorder.start(); if(status)status.textContent='Recordingâ€¦ tap to stop'; action.textContent='Stop recording';
  ...
  else if(action.dataset.action==='play'){action.textContent=action.textContent==='âšâš'?'â–¶':'âšâš';notify(action.textContent==='âšâš'?'Playing voice note':'Voice note paused');}
...
document.addEventListener('change',async e=>{
  ...
  if(e.target.id==='file-upload'){
    ...
    const chat=chats[state.activeChat]||chats[0];
    try { const url=await uploadAsset(file,`users/${state.userId}/saved/${safeStorageName(file)}`); chat.messages.push(['mine',`File uploaded: ${file.name} Â· ${size} (${url})`,'Now']); chat.preview=`Uploaded ${file.name}`; notify('File saved'); } catch(error) { notify(error.message||'Unable to upload your file.'); } finally { e.target.value=''; render(); }
  }
});
...
async function connectFirebase() {
  try {
    stopCloudPosts=await observePosts(cloudPosts => { posts.splice(0,posts.length,...cloudPosts.map(post => ({ ...post, id:String(post.id), ago:post.createdAt?.toDate ? 'just now' : post.ago||'just now', comments:post.comments||0 }))); render(); }, error => console.warn('Unable to load posts', error));
