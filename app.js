...
document.addEventListener('submit',async e=>{
  ...
  if(e.target.id==='edit-channel-form'){
    ...
    const data=new FormData(e.target);const name=String(data.get('name')||'').trim();const image=data.get('channelImage');const access=e.target.querySelector('[data-visibility].selected')?.dataset.visibility||channel.access||'Public';const submit=e.target.querySelector('button.primary');if(access==='Private'&&state.subscriptionPlan==='Free'){notify('Private channels require Student+ or Power.');return;}submit.disabled=true;
